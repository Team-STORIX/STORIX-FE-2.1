...
const checkPinkIcon = require("../../../../assets/icons/search/icon-check.svg");
...
export function LibraryScreen() {
  ...
  const works = useMemo<LibraryUiWork[]>(() => {
    const items =
      reviewQuery.data?.pages.flatMap((page) => page.result.content) ?? [];
  ...
  const worksCount =
    reviewQuery.data?.pages?.[0]?.totalReviewCount ?? works.length;
  ...
  return (
    <View style={[styles.screen, { paddingTop: insets.top }]}>
      <View style={styles.headerLayer}>
        <LibraryHeader
          onSearchPress={() => router.push("/library/search" as never)}
        />
        ...
        <View style={styles.controlsWrap}>
          ...
          <View style={styles.rightControls}>
            ...
            <Pressable
              ...
              onPress={() =>
                setMode((prev) => (prev === "list" ? "gallery" : "list"))
              }
              accessibilityRole="button"
              accessibilityLabel={
                mode === "list" ? "갤러리형 보기" : "리스트형 보기"
              }
            >
              <Image
